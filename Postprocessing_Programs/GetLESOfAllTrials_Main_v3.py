...
class GetLESOfAllTrials_Main:
    ...
    def run(self):
        ...
        for dirpath, dirnames, filenames in os.walk(path):
            ...
            if any([(dirname.startswith('Trial') and dirname.replace('Trial','').isdigit()) for dirname in dirnames]):
                ...
                toString += 'Q1: ' + str(scipy.stats.mstats.mquantiles(all_dirs, [0.25])) + '\n'
                toString += 'Q2: ' + str(scipy.stats.mstats.mquantiles(all_dirs, [0.50])) + '\n'
                toString += 'Q3: ' + str(scipy.stats.mstats.mquantiles(all_dirs, [0.75])) + '\n'
                toString += '90th Percentile: ' + str(scipy.stats.mstats.mquantiles(all_dirs, [0.9])) + '\n'
                toString += '99th Percentile: ' + str(scipy.stats.mstats.mquantiles(all_dirs, [0.99])) + '\n'
                #toString += str(all_dirs) + '\n'
                print(toString)
                toString += '******************************************************************************\n'
                toString += '******************************************************************************\n'
    ...
    def get_LES_Details(self, dirpath, dirnames, max_generation_to_survey, rounding):
                ...
                with open(filepath,'r') as DETAILS_FILE:
                    for line in DETAILS_FILE:
                        ...
                        if line.endswith('None'):
                            got_to_trial = True
                            break
                        energy = float(line.rstrip().split()[3][:-1])
                        gen = int(line.rstrip().split()[1][:-1])
			clu_dir = gen
                ...
                """
                """
                #irrelavant for a basin hopping algorithm.
                if gen == max_generation_to_survey:
        ...
        for index in range(number_of_dirnames):
            ...
            LES_clu_dir, LES_gen, LES_energy = Lowest_Energy_from_Trial(dirpath+'/'+dirname+'/log.txt',rounding)
