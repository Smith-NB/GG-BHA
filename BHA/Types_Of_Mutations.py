...
from BHA.ExternalDefinitions import InclusionRadiusOfCluster, is_position_already_occupied_by_an_atom_in_Cluster
from ase import Atoms
import numpy as np


def cartesian_displacement(cluster_to_mutate, step_width):
	'''
	...
	'''
	disp = np.random.uniform(-1., 1., (len(cluster_to_mutate), 3))
	disp *= step_width
	return disp

def geometric_center_displacement(cluster_to_mutate, alpha_min, alpha_max, weight=2):
	'''
	...
	:rtype list [[x1, y1, z1], ..., [xn, yn, zn]]
	'''
	radius = InclusionRadiusOfCluster(cluster_to_mutate)
	center_of_mass = cluster_to_mutate.get_center_of_mass()

	#calculate radial displacement direction and magnitude
	spherical_disp = np.random.uniform(0., 360., (len(cluster_to_mutate),2)) 	#angles
	dist = []
	
	#Scale displacement magnitude by min/max distance and distance to center of mass
	for i in range(len(cluster_to_mutate)):
		distance_to_center = distance_between_points(cluster_to_mutate[i].position, center_of_mass)
		dist.append((alpha_max - alpha_min) * (distance_to_center/radius)**weight + alpha_min)		#scale distance
...
def angular_displacement(cluster_to_mutate, n_atoms_to_move):
	'''
	...
	if n_atoms_to_move > len(cluster_to_mutate): n_atoms_to_move = len(cluster_to_mutate)

	radius = InclusionRadiusOfCluster(cluster_to_mutate)
	center_of_mass = cluster_to_mutate.get_center_of_mass()
	spherical_disp = np.random.uniform(0., 360., (n_atoms_to_move, 2))
	cart_disp = []
	for i in range(n_atoms_to_move):
		cart_disp.append(convert_spherical_to_cartesian(radius, spherical_disp[i][0], spherical_disp[i][1]))
		cart_disp[i] += center_of_mass
	return cart_disp
...
def randomMutate(boxtoplaceinlength,vacuumAdd,cluster_makeup=None,cluster_to_mutate=None,percentage_of_cluster_to_randomise=None):
	...
	if not cluster_makeup == None and (cluster_to_mutate == None or percentage_of_cluster_to_randomise == None):
		...
		mutant = Atoms(cluster_chemical_formula); nAtoms = len(mutant); atoms_to_randomise = range(nAtoms) # randomise all atoms in the cluster.
