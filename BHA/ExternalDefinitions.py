...
def get_coordination_number(cluster, max_distance_between_atoms):
	coordination_numbers = []

	for i in range(len(cluster)):
		coordination_numbers.append([i, 0])

	for i in range(len(cluster)):
		for j in range(i+1, len(cluster)):
			distance = get_distance(cluster[i].position, cluster[j].position)
			if distance < max_distance_between_atoms:
				coordination_numbers[i][1] += 1
				coordination_numbers[j][1] += 1

	return coordination_numbers
def get_neighbours_list(cluster, max_distance_between_atoms):
	'''
...
def Exploded(cluster, max_distance_between_atoms):
	...
	* Write this later
	...
	neighbours_list = get_neighbours_list(cluster, max_distance_between_atoms)
